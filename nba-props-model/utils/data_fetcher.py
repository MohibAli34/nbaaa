...
def fetch_fanduel_lines(event_id, api_key="f6aac04a6ab847bab31a7db076ef89e8"):
    ...
    if not event_id or not api_key:
        safe_print("❌ Missing event_id or API key")
        return {}
    ...
    try:
        ...
        safe_print(f"🔍 Fetching player props for event {event_id}...")
        ...
        if isinstance(data, list):
            if len(data) == 0:
                safe_print("⚠️ No odds data returned for this event")
                return {}
            data = data[0]

        bookmakers = data.get("bookmakers", [])
        safe_print(f"✅ Got odds response with {len(bookmakers)} bookmakers")
        ...
        for bookmaker in bookmakers:
            ...
            safe_print(f"📊 Processing FanDuel odds with {len(bookmaker.get('markets', []))} markets")
            ...
            for market in bookmaker.get("markets", []):
                ...
                for outcome in outcomes:
                    try:
                        ...
                        outcome_type = outcome.get("name", "").strip().lower()
                        ...
                        if outcome_type == "over":
                            player_props[player_name][stat_code]["over_price"] = outcome.get("price")
                        elif outcome_type == "under":
                            player_props[player_name][stat_code]["under_price"] = outcome.get("price")

                    except Exception as e:
                        safe_print(f"⚠️ Error processing outcome: {e}")
                        continue

        safe_print(f"✅ Parsed props for {len(player_props)} players")
            
        return player_props

    except Exception as e:
        safe_print(f"❌ Error fetching FanDuel lines: {e}")
        import traceback
...
def get_event_id_for_game(
    ...
    try:
        ...
        safe_print(f"🔍 Looking for event: {away_team} @ {home_team}")
        response = requests.get(url, params=params, timeout=10)

        if response.status_code != 200:
            safe_print(f"❌ Events API error: {response.status_code}")
            return None
        ...
        for event in events:
            ...
            if (home_full in event_home) and (away_full in event_away):
                ...
                safe_print(f"✅ Found event ID: {event_id}")
                return event_id

        safe_print(f"⚠️ No event found for {away_team} @ {home_team}")
        return None

    except Exception as e:
        safe_print(f"❌ Error getting event ID: {e}")
        return None


def get_player_fanduel_line(player_name, stat, odds_data):
    """
    Look up a player's prop line in the odds data with flexible name matching.
    
    Args:
        player_name: Name of the player to look up
        stat: The stat code (PTS, REB, AST, FG3M, PRA)
        odds_data: Dictionary of odds data from fetch_fanduel_lines()
        
    Returns:
        dict: Line data if found, None otherwise
    """
    if not odds_data:
        return None
        
    # First try exact match
    if player_name in odds_data and stat in odds_data[player_name]:
        return odds_data[player_name][stat]
        
    # Try flexible name matching
    for name, props in odds_data.items():
...
def get_player_position(player_id, season="2024-25"):
    ...
    def simplify_position(pos_string):
        ...
        safe_print(f"🔍 Parsing position: '{pos_string}'")
        ...
        if "-" in pos_string:
            ...
            safe_print(f"   → Multi-position: {pos_string}")
            safe_print(f"   → Using PRIMARY (first): {first_pos}")

            if "G" in first_pos:  # G, PG, SG
                safe_print(f"   → Primary is Guard: returning G")
                return "G"
            elif "C" in first_pos:  # C
                safe_print(f"   → Primary is Center: returning C")
                return "C"
            else:  # F, PF, SF
                safe_print(f"   → Primary is Forward: returning F")
                return "F"

        # Single position fallbacks
        if "G" in pos_string:  # G, PG, SG
            safe_print(f"   → Guard: returning G")
            return "G"
        elif "C" in pos_string:  # C
            safe_print(f"   → Center: returning C")
            return "C"
        else:  # F, PF, SF
            safe_print(f"   → Forward: returning F")
            return "F"
    ...
    try:
        ...
        if meta and meta.get("position"):
            ...
            if cached_simplified:
                safe_print(f"   ✅ (DB cached) Position: {cached_simplified}\n")
                return cached_simplified
    except Exception as e:
        safe_print(f"   ⚠️ DB metadata lookup failed: {e}")
    ...
    try:
        ...
        try:
            ...
            safe_print(f"   ⚠️ Roster method failed: {e}")
        ...
        if not final_position_letter:
            try:
                ...
                safe_print(f"   ⚠️ API method failed: {e}")
        ...
        if not final_position_letter:
            try:
                ...
                safe_print(f"   ⚠️ Static method failed: {e}")

        # If STILL nothing, default to Forward
        if not final_position_letter:
            safe_print("   ⚠️ All methods failed - defaulting to F\n")
        ...
        try:
            ...
            safe_print(f"   ⚠️ could not save player_metadata: {e}")

        safe_print(f"   ✅ Position: {final_position_letter}\n")
        return final_position_letter

    except Exception as outer_e:
        safe_print(f"   ❌ ERROR determining position: {outer_e}")
        # last-resort safety
        return "F"
...
# -------------------------------------------------
# Starter detection / projection
# -------------------------------------------------
@st.cache_data(ttl=3600)
def get_team_starters(team_abbrev, season="2024-25", fallback_season=None):
    """
    Get projected starters for a team based on recent player game logs.
    Uses a simpler approach: check recent player game logs and identify players
    who consistently play significant minutes.
    Returns a set of player_ids who are likely starters.
    
    If fallback_season is provided and current season has insufficient data,
    will try fallback_season.
    """
    try:
        all_teams = teams.get_teams()
        ...
        if roster_df.empty and fallback_season:
            # Try fallback season for roster
            roster = commonteamroster.CommonTeamRoster(team_id=team_id, season=fallback_season)
            roster_df = roster.get_data_frames()[0]
        ...
        # For each player on the roster, check their recent game logs
        for _, player_row in roster_df.iterrows():
            ...
            for try_season in seasons_to_try:
                try:
                    ...
                    games_df = player_gamelog.get_data_frames()[0]
                    
                    if not games_df.empty and len(games_df) >= 3:
            ...
            # Check if player has been playing consistently
            if len(recent_games) < 2:  # Need at least 2 games (more lenient)
                continue
            
            # Parse minutes from MIN column
            ...
            try:
                # Calculate average minutes in recent games
                if "MIN" in recent_games.columns:
                    minutes_list = [parse_minutes(m) for m in recent_games["MIN"]]
                    # Filter out games where player didn't play (0 minutes)
                    active_minutes = [m for m in minutes_list if m > 0]
                    
                    if len(active_minutes) == 0:
                        continue
                    
                    avg_minutes = sum(active_minutes) / len(active_minutes)
                    games_played = len(active_minutes)
                    total_games = len(minutes_list)
                    
                    # Starter criteria (more lenient):
                    # 1. Average > 18 minutes in games they played, OR
                    # 2. Played in 5+ of last 10 games with avg > 12 minutes, OR
                    # 3. Played in most recent 3+ games with avg > 15 minutes
                    
                    recent_3_avg = sum(active_minutes[:3]) / min(3, len(active_minutes)) if len(active_minutes) >= 3 else 0
                    
                    if avg_minutes > 18:  # High average minutes = likely starter
                        starter_candidates[player_id] = avg_minutes
                    elif games_played >= 5 and avg_minutes > 12:  # Consistent rotation player
                        starter_candidates[player_id] = avg_minutes
                    elif len(active_minutes) >= 3 and recent_3_avg > 15:  # Recently active starter
                        starter_candidates[player_id] = recent_3_avg
            
            except Exception as e:
                # Skip players we can't get data for
                continue
        
        # Always take top 10 by average minutes (ensures we get reasonable number)
        if len(starter_candidates) > 0:
            # Sort by average minutes and take top 10 (or all if less than 10)
            sorted_candidates = sorted(starter_candidates.items(), key=lambda x: x[1], reverse=True)
            num_starters = min(10, len(sorted_candidates))
            starters = {pid for pid, _ in sorted_candidates[:num_starters]}
        else:
            starters = set()
        
        safe_print(f"Found {len(starters)} likely starters for {team_abbrev} (from {len(starter_candidates)} candidates)")
        if len(starters) > 0:
            # Debug: print first few starter names
            starter_names = []
            for pid in list(starters)[:5]:
                player_info = roster_df[roster_df["PLAYER_ID"] == pid]
                if not player_info.empty:
                    starter_names.append(player_info.iloc[0]["PLAYER"])
            safe_print(f"Sample starters: {', '.join(starter_names)}")
        else:
            safe_print(f"No starters found for {team_abbrev} - may need to adjust criteria")
        return starters
        
    except Exception as e:
        safe_print(f"Error getting team starters: {e}")
        import traceback
        traceback.print_exc()
        return set()
...
# -------------------------------------------------
def get_players_by_team(team_abbrev, season="2024-25", include_starters=True):
    """
    Get all players who have played for a specific team in a season.
    Returns DataFrame with: player_id, full_name, position, starter (bool)
    
    If include_starters=True, will try to identify and mark starters.
    """
    try:
        all_teams = teams.get_teams()
        team_info = [t for t in all_teams if t["abbreviation"] == team_abbrev]
        ...
        if not df.empty:
            # Get starters if requested
            starters_set = set()
            if include_starters:
                try:
                    # Determine fallback season (previous year)
                    fallback_season = None
                    if season and "-" in season:
                        parts = season.split("-")
                        prev_year = int(parts[0]) - 1
                        fallback_season = f"{prev_year}-{str(prev_year + 1)[-2:]}"
                    starters_set = get_team_starters(team_abbrev, season=season, fallback_season=fallback_season)
                except Exception as e:
                    safe_print(f"Warning: Could not get starters for {team_abbrev}: {e}")
                ...
                players_list.append(
                    {
                        ...
                        "starter": player_id in starters_set,
                    }
                )
        ...
        safe_print(f"Error fetching team roster: {e}")
        return pd.DataFrame()

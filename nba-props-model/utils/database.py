...
def get_cached_player_game_logs(player_id, season):
    ...
    try:
        ...
        safe_print(f"✅ Loaded {len(df)} games for player {player_id} from cache ({season})")
        return df
        
    except Exception as e:
        safe_print(f"❌ Error loading from cache: {e}")
        conn.close()
        return None
...
def save_player_game_logs(player_id, player_name, season, game_logs_df):
    ...
    for _, row in game_logs_df.iterrows():
        try:
            ...
            safe_print(f"⚠️ Error saving game row for {player_id}: {e}")
            continue
    
    conn.commit()
    conn.close()
    safe_print(f"💾 Saved {len(game_logs_df)} games for {player_id} / {season}")
...
def save_team_stats(team_abbrev, season, stats_df):
    ...
    conn.commit()
    conn.close()
    safe_print(f"💾 Saved team stats for {team_abbrev} / {season}")
...
def get_cached_team_stats(season):
    ...
    try:
        ...
        if datetime.now() - latest_update > timedelta(hours=24):
            safe_print("⚠️ Team stats cache is stale (>24h)")
            return None
        ...
        safe_print(f"✅ Loaded cached team stats for season {season} ({len(df)} rows)")
        return df
        
    except Exception as e:
        safe_print(f"❌ Error loading cached team stats: {e}")
        return None
...
def save_defense_vs_position(df):
    ...
    conn.commit()
    conn.close()
    safe_print(f"💾 Saved defense vs position data ({len(df)} rows)")
...
def get_cached_defense_vs_position():
    ...
    try:
        ...
        if datetime.now() - latest_update > timedelta(days=7):
            safe_print("⚠️ Defense-vs-position cache is stale (>7d)")
            return None
        ...
        safe_print(f"✅ Loaded defense vs position from cache ({len(df)} rows)")
        return df
        
    except Exception as e:
        safe_print(f"❌ Error loading defense-vs-position cache: {e}")
        conn.close()
        return None
...
def save_player_metadata(player_id, player_name, position, team):
    ...
    conn.commit()
    conn.close()
    safe_print(f"💾 Saved player_metadata for {player_id}: {player_name}, {position}, {team}")
...
def clear_cache():
    ...
    if os.path.exists(DATABASE_PATH):
        ...
        safe_print("✅ Cache cleared (DB file deleted)")
        init_database()
    else:
        safe_print("⚠️ No cache DB to clear")
...
def clear_old_seasons(keep_seasons):
    ...
    safe_print(f"🧹 Removed data from old seasons ({deleted} records)")
